  ...
  (factory((global.d3 = global.d3 || {}),global.d3));
	...
	function geoTile() {
		...
		geoTile.size = function(_) {
		    // Can be passed a selected element, SVG element, or array of width/height.
			if(arguments.length) {
				if(_ instanceof d3.selection) {
					let bb = _.node().getBoundingClientRect();
					w = bb.width || 960;
					h = bb.height || 500;
				}
				else if (_ instanceof SVGElement){
		...
		geoTile.fit = function(f) {
			return arguments.length ? (p.fitSize([w,h],toGeojson(f)),tx = p.translate()[0],ty = p.translate()[1],pk = p.scale(), geoTile) : "n/a";
		};
		geoTile.fitMargin = function(m,f) {
			return arguments.length > 1 ? (p.fitExtent([[m,m],[w-m,h-m]],toGeojson(f)), tx = p.translate()[0],ty = p.translate()[1],pk = p.scale(), geoTile) : "n/a";
		};
